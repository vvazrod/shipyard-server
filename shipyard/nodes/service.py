...
class NodeService():
    ...
    def get_all() -> List[Node]:
        return Node.Schema().load(nodes.find(), many=True)

    @staticmethod
    def get_by_id(node_id: str) -> Node:
        result = nodes.find_one({'_id': ObjectId(node_id)})
        if result is None:
            return None
    ...
    def get_by_name(node_name: str) -> Node:
        result = nodes.find_one({'name': node_name})
        if result is None:
            return None
        return Node.Schema().load(result)
    ...
    def create(new_node: Node) -> str:
        result = nodes.find_one({'name': new_node.name})
        if result is not None:
            raise ValueError('A node already exists with the given name.')
        new_id = nodes.insert_one(Node.Schema(
            exclude=['_id']).dump(new_node)).inserted_id
        return str(new_id)

    @staticmethod
    def delete(node_id: str) -> Node:
        result = nodes.find_one_and_delete({'_id': ObjectId(node_id)})
        if result is None:
            return None
        return Node.Schema().load(result)
